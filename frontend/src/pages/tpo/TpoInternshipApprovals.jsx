...
const TpoInternshipApprovals = () => {
  ...
  const updateStatus = (id, newStatus) => {
    setInternships(internships.map(i => i._id === id ? { ...i, status: newStatus } : i));
  };
            ...
            {pending.map(intern => (
              <div key={intern._id} className="bg-surface-container p-6 rounded-2xl flex items-center justify-between">
                <div>
                  ...
                  <p className="text-secondary text-sm">{intern.role} • {intern.duration_months} Months</p>
                </div>
                <div className="flex gap-4">
                  <button 
                    onClick={() => updateStatus(intern._id, 'Approved')}
                    className="bg-primary-container hover:scale-105 text-white px-6 py-2 rounded-xl text-xs font-bold uppercase transition-all shadow-lg"
                  >
                    Approve
                  </button>
                  <button 
                    onClick={() => updateStatus(intern._id, 'Rejected')}
                    className="bg-error/20 hover:bg-error text-error hover:text-white border border-error/30 px-6 py-2 rounded-xl text-xs font-bold uppercase transition-all"
                  >
                    Reject
                  </button>
                </div>
