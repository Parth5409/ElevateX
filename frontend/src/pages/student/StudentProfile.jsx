    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <section className="flex justify-between items-end mb-12">
        ...
        <div className="flex gap-4">
          ...
          <button className="px-8 py-3 bg-primary-container text-on-primary-fixed font-bold rounded-xl text-sm shadow-xl shadow-primary-container/20 hover:scale-[1.02] transition-transform active:scale-95">Save Profile</button>
        </div>
      </section>
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-4 flex flex-col gap-6">
          <div className="glass-card p-8 rounded-2xl border border-white/5 flex-1">
            ...
            <div className="flex flex-wrap gap-2">
              {profile.skills.map((skill, i) => (
                <span key={i} className="px-4 py-2 bg-primary-container/10 border border-primary-container/20 text-primary-container rounded-full text-xs font-bold hover:bg-primary-container hover:text-white transition-all cursor-pointer">
                  {skill}
                </span>
              ))}
            </div>
            <button className="mt-8 w-full py-3 border border-dashed border-white/10 rounded-xl text-xs font-bold text-tertiary hover:border-primary-container/40 hover:text-primary-container transition-all">
              + Add Skill Parameter
            </button>
          </div>
        </div>
        ...
        <div className="col-span-12 glass-card rounded-2xl border border-white/5 overflow-hidden">
          <div className="p-8 border-b border-white/5 flex justify-between items-center bg-white/5">
            ...
            <div className="flex gap-2">
              <span className="bg-white/10 text-[10px] font-bold px-3 py-1 rounded-full text-tertiary">{profile.projects.length} REPOSITORIES ACTIVE</span>
            </div>
          </div>
