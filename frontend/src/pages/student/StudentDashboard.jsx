...
import { fetchProfile, fetchNotifications } from '../../store/slices/studentSlice';

const StudentDashboard = () => {
  const dispatch = useDispatch();
  const { user } = useSelector((state) => state.auth);
  const { profile, notifications, loading } = useSelector((state) => state.student);
  ...
  const profileCompletion = profile ? 85 : 0; // Simplified logic or derived from profile completeness
  const newMatchesCount = notifications.filter(n => n.type === 'Invite').length;
    ...
    <div className="grid grid-cols-12 gap-8">
        ...
        <section className="glass-card rounded-2xl p-8 relative overflow-hidden flex items-end min-h-[320px]">
          ...
          <div className="relative z-10 w-full flex justify-between items-end">
            <div className="max-w-md">
              ...
              <h2 className="text-5xl font-black tracking-tighter text-white leading-none capitalize">Welcome back, {firstName}.</h2>
              <p className="text-secondary mt-4 text-lg">Your kinetic engine is optimized. {newMatchesCount} new placement matches are ready for review.</p>
            </div>
            <div className="flex flex-col items-center">
              <div className="relative h-28 w-28 flex items-center justify-center">
                <svg className="h-full w-full -rotate-90">
                  <circle cx="56" cy="56" fill="transparent" r="50" stroke="rgba(250,91,49,0.1)" strokeWidth="8"></circle>
                  <circle cx="56" cy="56" fill="transparent" r="50" stroke="#fa5b31" strokeDasharray="314" strokeDashoffset={314 - (314 * profileCompletion) / 100} strokeLinecap="round" strokeWidth="8"></circle>
                </svg>
                <span className="absolute text-2xl font-black text-white">{profileCompletion}%</span>
              </div>
        ...
        <div className="glass-card rounded-2xl p-6 h-full flex flex-col">
          ...
          <div className="space-y-6 flex-1 overflow-y-auto pr-2 custom-scrollbar">
            {loading && notifications.length === 0 ? (
               <p className="text-secondary text-[10px] font-bold uppercase tracking-widest text-center mt-10">Resonating Stream...</p>
            ) : notifications.length === 0 ? (
               <p className="text-secondary text-[10px] font-bold uppercase tracking-widest text-center mt-10">Stream Silent</p>
            ) : (
              notifications.map((notif, index) => (
                <div key={notif._id} className={`relative pl-6 border-l ${index === 0 ? 'border-primary-container/20' : 'border-white/10'} pb-2`}>
                  <div className={`absolute -left-1.5 top-0 h-3 w-3 rounded-full ${index === 0 ? 'bg-primary-container' : 'bg-surface-container-highest'}`}></div>
                  <p className={`text-[10px] font-bold ${index === 0 ? 'text-primary-container' : 'text-tertiary'} uppercase tracking-widest`}>
                    {new Date(notif.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </p>
                  <p className="text-sm font-bold text-white mt-1">{notif.title}</p>
      ...
      <div className="col-span-12 grid grid-cols-4 gap-8">
        ...
        <div className="bg-surface-container-low rounded-2xl p-6 border-b-2 border-white/5">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-black text-white">{newMatchesCount.toString().padStart(2, '0')}</span>
            {newMatchesCount > 0 && <span className="text-[10px] font-bold text-primary-container px-1.5 py-0.5 bg-primary-container/10 rounded">NEW</span>}
          </div>
        </div>
      </div>
    </div>
