...
import { fetchNotifications } from '../../store/slices/studentSlice';

const StudentAIMentor = () => {
  const dispatch = useDispatch();
  const { notifications, loading } = useSelector((state) => state.student);
      ...
      <div className="flex justify-between items-end mb-12">
        ...
        <div className="flex gap-4">
          <button 
            ...
            disabled={loading}
            className="px-6 py-2 bg-primary-container text-on-primary-container font-bold text-xs uppercase tracking-widest rounded-lg hover:scale-[1.02] transition-transform disabled:opacity-50"
          >
            {loading ? 'SYNCING ENGINE...' : 'Refresh Engine'}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-8 space-y-6">
          ...
          {loading && notifications.length === 0 ? (
            <p className="text-secondary text-center py-10 uppercase text-[10px] font-bold tracking-widest">Resonating with Neural Feed...</p>
          ) : notifications.length === 0 ? (
            <p className="text-secondary text-center py-10">No intelligence nodes found in your orbit.</p>
          ) : (
              ...
              <div key={notif._id} className={`glass-card p-6 rounded-xl relative overflow-hidden group ${notif.type === 'Invite' ? 'ai-glow border-l-4 border-primary' : 'border border-white/5'}`}>
                <div className="absolute top-0 right-0 p-6">
                  <div className="flex flex-col items-end">
                    {notif.matchScore ? (
                      <span className="text-4xl font-black text-primary leading-none">{notif.matchScore}%</span>
                    ) : (
                      <span className="text-xs font-bold text-secondary uppercase tracking-widest opacity-50">Node Validated</span>
                    )}
                    <span className="text-[10px] font-bold text-secondary uppercase tracking-widest mt-1">
                      {notif.date ? new Date(notif.date).toLocaleDateString() : 'REALTIME'}
                    </span>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    {notif.missing_skills && notif.missing_skills.length > 0 && (
